...
def compute_vi(clusterings):
    comparisons = []
    for (label1, cl1), (label2, cl2) in itertools.combinations(clusterings.items(), 2):
        vi = igraph.compare_communities(cl1, cl2, method='vi')
        comparisons.append( (label1, label2, vi) )
    return comparisons
